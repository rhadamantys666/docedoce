...
export function aleatorio (lista){
    const posição = Matemática.piso (matemática.random()* lista.comprimento);
...
function iniciaJogo() {
    atual = 0;
    historiaFinal = "";
    telaInicial.estilo.display = 'nenhum';
...
function mostraPergunta() {
    if (atual >= perguntas.comprimento) {
        mostraResultado();
        retornar;
    }
    perguntaAtual = perguntas[atual];
    caixaPerguntas.textContent = perguntaAtual.enunciado;
    caixaAlternativas.conteúdo de texto = "";
    mostraAlternativas();
}
...
function respostaSelecionada(opcaoSelecionada) {
    const afirmações = aleatorio(opcaoSelecionada.afirmação);
